...
--sw
NPL.load("(gl)script/Truck/sha1/sha1.lua");
...
LOG.std(nil, "debug", "develop", "GameLogic Loaded");
-- call this when user first enters a game world.
function GameLogic.Init(worldObj)
    LOG.std(nil, "debug", "develop", "GameLogic Initing");
    ...
    LOG.std(nil, "debug", "develop", "Setting Game Mode by GameLogic.IsReadOnly()");
    if(GameLogic.IsReadOnly()) then
        LOG.std(nil, "debug", "develop", "Readonly: game");
        attr:SetField("IsReadOnly", true);
    else
        LOG.std(nil, "debug", "develop", "Not ReadOnly: editor");
    ...
    -- world manager
    LOG.std(nil, "debug", "develop", "GameLogic: initing world");
...
function GameLogic.OnBeforeLoadBlockRegion(bContinue, region_x, region_y)
    ...
    NPL.load("(gl)script/Truck/Framework/TruckFilters.lua");
    local TruckFilters = commonlib.gettable("Mod.Truck.Framework.TruckFilters");
    bContinue = TruckFilters.OnBeforeLoadBlockRegion(bContinue, region_x, region_y);
    return bContinue;
end
...
function GameLogic.LoadGame()
    ...
    --Cellfy: this call may cause crash when running in Parallels Desktop 11 Win7, it could be comment out in this case since it's not a necessary feature
    ...
    ParaTerrain.GetBlockAttributeObject():SetField("UseSunlightShadowMap",SystemSettingsPage.setting_ds["sunlight_shadow"]);
    GameLogic.GetShaderManager():SetShaders(math.min(2,SystemSettingsPage.setting_ds["emulational_lighting_value"]));
    --ParaTerrain.GetBlockAttributeObject():SetField("UseLinearTorchBrightness",SystemSettingsPage.setting_ds["emulational_lighting_value"]>=3);
    ...
    WorldCommon.GetWorldInfo().rendermethod=SystemSettingsPage.setting_ds["emulational_lighting_value"];
    local CameraController = commonlib.gettable("MyCompany.Aries.Game.CameraController")
...
function GameLogic.QuickSave(saveCallBack)
    GameLogic.SaveAll(true,saveCallBack);
end
...
function GameLogic.SaveAll(bSaveToLastSaveFolder,saveCallBack)
    ...
    LOG.std(nil, "debug", "develop", "Cloud uploading starts here");
    


	local upload = function (callback)
		local planetMgr = commonlib.gettable("Mod.Truck.Game.World.PlanetManager");
		...
		NPL.load("(gl)script/Truck/Settings.lua");
		local Settings = commonlib.gettable("Mod.Truck.Settings");
		for k,v in pairs(Settings.saveCraftFiles) do
			planet:refreshLocalFile(v);
		end
        for k,v in pairs(Settings.needCheckSaveCraftFiles) do
            local file_exist = ParaIO.DoesFileExist(planet:getFolderDir()..v, false);
            if file_exist then
                planet:refreshLocalFile(v);
            end
        end
		...
		local files = commonlib.Files.Find(nil,planet:getFolderDir(),folderDepth,fileMaxnum,"*.bmax");
		for k,v in pairs(files) do
			planet:refreshLocalFile(v.filename);
		end

		planet:upload(callback);
	end
...
function GameLogic.Exit()
    ...
    MyCompany.Aries.Game.MainLogin:ShowLoginBackgroundPage(true,true,true,true);
...
function GameLogic.EnterCommunityMode()
    GameLogic.SetMode("community", true);
    if(Truck.QuickBtnBarPage) then
        Truck.QuickBtnBarPage:Refresh(0);
    end
end
...
function GameLogic.DoJump()
    ...
    if(true) then
        if(GameLogic.GetPlayerController():IsInWater()) then
            ...
            if(not GameLogic.options.CanJump) then
                return false;
            elseif(GameLogic.GetPlayerController():IsInAir() and not player:IsFlying()) then
                    return false;
            end
        end
    end
    ...
    if( options.double_click_flying and (commonlib.TimerManager.GetCurrentTime() - last_jump_tick)< options.double_click_ticks) then
        ...
        if(not ParaUI.IsKeyPressed(DIK_SCANCODE.DIK_W)) then
            ...
            --CommandManager:RunCommand("/sound jump gameassets/audios/voices/playerJump.mp3 0");
        end
    else
        local player = EntityManager.GetFocus();
        if(player) then
...
function GameLogic.ShowMsg(text, level)
    ...
    if(level >= 1) then
        if(GameLogic.error_count == 1) then
            ...
            UIManager.createUI("ChatWindow",ui_main);
        end
    ...
    if(entity and not entity:IsControlledExternally()) then
        if(bRunning~= GameLogic.IsRunning) then
            if( (GameLogic.options.AllowRunning and bRunning) or not bRunning) then
                ...
                if(bRunning) then
                    CameraController.AnimateFieldOfView(MyCompany.Aries.Player.LoadLocalData("SystemSettingsPage_Fov",1.04,true)*0.8, options.slow_speed_fov);
                else
                    CameraController.AnimateFieldOfView(MyCompany.Aries.Player.LoadLocalData("SystemSettingsPage_Fov",1.04,true), options.slow_speed_fov);
                end
            end
        end
    end
...
function GameLogic.ToggleDesktop(name)
    if(name == "esc") then
        ...
        local ui_main = UIManager.getUI("UIMain")
        if ui_main then
            local saving = ModuleManager.checkModule("Saver");
            -- ui_main:showEscPage(saving);
            NPL.load("(gl)script/Truck/EscFrameManager.lua")
            local EscFrameManager = commonlib.gettable("Mod.Truck.EscFrameManager")
            --EscFrameManager.LeavePlanet(saving)
        end
    elseif(name == "builder") then
        if(GameMode:IsUseCreatorBag()) then
            -- NPL.load("(gl)script/Truck/CreatorDesktop.lua");
            -- local CreatorDesktop = commonlib.gettable("Mod.Truck.UI.CreatorDesktop");
            -- CreatorDesktop.ShowPage();
            -- NPL.load("(gl)script/Truck/BuilderFramePage.lua");
            -- local BuilderFramePage = commonlib.gettable("Mod.Truck.UI.BuilderFramePage");
            -- BuilderFramePage.ShowPage();

            local ui_main = UIManager.getUI("UIMain")
            if (not UIManager.getUI("BuilderFramePage")) then
                UIManager.createUI("BuilderFramePage",ui_main);
            else
                UIManager.destroyUI("BuilderFramePage")
            end
        else
...
function GameLogic.AddBBS(channel, text, max_duration, color)
  if text then
    text=string.gsub(text,L"目前只有Server可以创建此类物品",L"目前只有房主可以创建此类物品");
  end
...
function GameLogic.ToggleCamera(IsFPSView)
	local mode = nil;
	if IsFPSView == true then
		mode = 1;
	elseif IsFPSView == false then
		mode = 0;
	end
...
function GameLogic.ActivateDefaultContext()
	local context = Truck:getInputListener();
	if GameLogic.GetMode()=="movie" then
    context=MyCompany.Aries.Game.AllContext:GetContext("movie");
  end
	if (context) then
		context:activate()
end
